...
public class MapService implements IMapService {
  ...
  private static String encodeParams(final Map<String, String> params) {
    final String paramsUrl =
        Joiner.on('&')
            .join( // получаем значение вида key1=value1&key2=value2...
                Iterables.transform(
                    params.entrySet(),
                    new Function<Map.Entry<String, String>, String>() {
                      ...
                      public String apply(final Map.Entry<String, String> input) {
                        String buffer =
                            input.getKey()
                                + // получаем значение вида key=value
                                '='
                                + URLEncoder.encode(
                                    input.getValue(),
                                    StandardCharsets
                                        .UTF_8); // кодируем строку в соответствии со стандартом
                        // HTML 4.01
                        return buffer;
                      }
                    }));
  ...
  public CoordinatesDto geoCoding(String address) {
    final String baseUrl =
        "https://nominatim.openstreetmap.org/search"; // путь к Geocoding API по HTTP
    ...
    Optional<String> optionalLon =
        Arrays.stream(strings)
            .filter(s -> s.contains("lon:"))
            .map((s -> s.split(":")))
            .map((s -> s[1]))
            .findFirst();
    Optional<String> optionalLat =
        Arrays.stream(strings)
            .filter(s -> s.contains("lat:"))
            .map((s -> s.split(":")))
            .map((s -> s[1]))
            .findFirst();
    ...
    return new CoordinatesDto(
        Double.valueOf(lat), Double.valueOf(lon)); // итоговая широта и долгота
  }

  public String geoDecoding(String lat, String lon) {
    final String baseUrl =
        "https://nominatim.openstreetmap.org/reverse"; // путь к Geocoding API по HTTP
    final Map<String, String> params = Maps.newHashMap();
    params.put("format", "json");
  ...
  public Double calcDistanceBetweenPoints(String departureAddress, String arrivalAddress) {
    ...
    final Double a =
        sin(dlat / 2) * sin(dlat / 2)
            + cos(deg2rad(departureAddressCoordinates.lat))
                * cos(deg2rad(arrivalAddressCoordinates.lat))
                * sin(dlng / 2)
                * sin(dlng / 2);
  ...
  public Double calcDistanceBetweenPoints(Airport departureAirport, Airport arrivalAirport) {
    final CoordinatesDto arrivalAddressCoordinates =
        new CoordinatesDto(
            Double.valueOf(arrivalAirport.getLat()), Double.valueOf(arrivalAirport.getLon()));
    final CoordinatesDto departureAddressCoordinates =
        new CoordinatesDto(
            Double.valueOf(departureAirport.getLat()), Double.valueOf(departureAirport.getLon()));
