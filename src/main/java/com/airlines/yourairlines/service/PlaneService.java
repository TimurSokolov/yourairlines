...
public class PlaneService extends CrudService<Plane> implements IPlaneService {
  @Autowired private IPlaneRepository planeRepository;
  ...
  private boolean isPlaneInFlightNow(Plane plane) {
    ArrayList<Boolean> arrayList =
        planeRepository.checkPlaneInFlightNow(plane.getId(), dayChangeService.getCurrentDate());
    return arrayList.stream().anyMatch(s -> s.equals(true));
  }

  private boolean isPlaneReservedOnFuture(Plane plane) {
    return flightService
        .calcLastReservedArrivalTime(plane)
        .isBefore(dayChangeService.getCurrentDate());
  }
  ...
  @Override
  public ArrayList<Plane> findSuitablePlanes(
      Long departureAirportId, Long arrivalAirportId, LocalDateTime departureTime) {
  ...
  public Plane findById(Long id) {
    ...
    return planeOptional.orElseThrow(
        () -> new NotFoundException(String.format("Борт с id = %s не найден", id)));
  }
  ...
  public void delete(Long id) {
    ...
    flightsToDelete.stream()
        ...
        .forEach(flightId -> flightService.delete(flightId));
