...
public interface IPlaneRepository extends IBaseRepository<Plane> {
  @Query(
      value =
          "(SELECT DISTINCT ON(flight.reserved_plane_id) plane.* FROM plane, flight "
  ...
  ArrayList<Plane> findSuitablePlanes(
      @Param("distance") Integer flightDistance,
      @Param("time") LocalDateTime requiredDateTime,
      @Param("airport") Long departureAirportId);

  @Query(
      value =
          "SELECT (:time, INTERVAL '0 sec') OVERLAPS (departure_time, arrival_time) FROM "
              + "(SELECT * FROM flight "
              + "WHERE reserved_plane_id = :plane) AS FlightsByReservedPlane",
      nativeQuery = true)
  ArrayList<Boolean> checkPlaneInFlightNow(
      @Param("plane") Long planeId, @Param("time") LocalDateTime currentDate);
}
